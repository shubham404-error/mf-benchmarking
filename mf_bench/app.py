    ...
    if scheme_code:
        with st.spinner("Fetching data..."):
            ...
            if nav_series is not None and not nav_series.empty:
                ...
                st.subheader(f"{meta.get('scheme_name', 'Unknown')}")
                st.write(f"**Category:** {cat_label} ({cat_string})")
                ...
                if selected_bench_ticker:
                    ...
                    if bench_series is not None:
                        # yfinance returns multi-index columns if downloaded in some ways, but single ticker is Series
                ...
                with col2:
                    st.write("### Risk Metrics (Annualized)")
                    risk_display = {
